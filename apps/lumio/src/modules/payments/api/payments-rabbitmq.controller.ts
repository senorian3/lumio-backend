...
export class PaymentsRabbitMQController {
  ...
  async handleSubscriptionCancelled(
    ...
    if (!messageId) {
      messageId = `subscription-cancelled-${Date.now()}`;
    }
    ...
    try {
      ...
      if (await this.idempotencyService.isMessageProcessed(messageId)) {
        channel.ack(originalMsg);
        this.appLogger.log(
          `Duplicate subscription cancelled message ignored: ${messageId}`,
          'Idempotency',
        );
        return;
      }
      ...
      channel.ack(originalMsg);
      this.appLogger.log(
        `Subscription cancelled message processed successfully: ${messageId}`,
        'PaymentsRabbitMQ',
      );
    } catch (error) {
      this.appLogger.error(
        `Error processing subscription cancelled message ${messageId}: ${error.message}`,
        error.stack,
        'PaymentsRabbitMQ',
      );
      ...
      if (retryCount >= maxRetries) {
        ...
        this.appLogger.warn(
          `Subscription cancelled message ${messageId} sent to DLQ after ${retryCount} retries`,
          'PaymentsRabbitMQ',
        );
      } else {
        // Increment retry count and requeue
        ...
        channel.nack(originalMsg, false, true);
        this.appLogger.warn(
          `Subscription cancelled message ${messageId} requeued for retry ${retryCount + 1}/${maxRetries}`,
          'PaymentsRabbitMQ',
        );
      }
    }
